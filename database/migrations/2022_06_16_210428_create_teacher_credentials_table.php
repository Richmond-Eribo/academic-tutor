...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create("teacher_credentials", function (Blueprint $table) {
            ...
            $table->boolean("right_to_work_isverified")->default(0);
            $table->string("dbs_certificate")->nullable();
            $table->boolean("dbs_certificate_isverified")->default(0);
            $table->string("educational_qualification")->nullable();
            $table->boolean("educational_qualification_isverified")->default(0);
            $table->string("qts")->nullable();
            $table->boolean("qts_isverified")->default(0);
            $table->string("passport_id_or_driver_license")->nullable();
            $table->boolean("passport_id_or_driver_license_isverified")->default(0);
            $table->string("passport_photo")->nullable();
            $table->boolean("passport_photo_isverified")->default(0);
            $table->string("proof_of_address")->nullable();
            $table->boolean("proof_of_address_isverified")->default(0);
            $table->string("national_insurance_number")->nullable();
            $table->boolean("national_insurance_number_isverified")->default(0);
            $table->string("permit_or_id")->nullable();
            $table->boolean("permit_or_id_isverified")->default(0);
